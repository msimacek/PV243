...
public class AuthorEndpoint {
    ...
    public Response create(@Valid Author entity) {
        em.persist(entity);
        return Response
                .created(UriBuilder.fromResource(AuthorEndpoint.class).path(String.valueOf(entity.getId())).build())
                .build();
    }
    ...
    public Response findById(@PathParam("id") Long id) {
        TypedQuery<Author> findByIdQuery = em.createQuery(
                "SELECT DISTINCT a FROM Author a LEFT JOIN FETCH a.books WHERE a.id = :entityId ORDER BY a.id",
                Author.class);
    ...
    public List<Author> listAll(@QueryParam("start") Integer startPosition, @QueryParam("max") Integer maxResult) {
        TypedQuery<Author> findAllQuery = em
                .createQuery("SELECT DISTINCT a FROM Author a LEFT JOIN FETCH a.books ORDER BY a.id", Author.class);
